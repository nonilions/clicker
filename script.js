...
const achTgl = document.getElementById('achievements-toggle');
const achPnl = document.getElementById('achievements-panel');
...
function showToast(t) {
    if (toast && tTxt) { tTxt.textContent = t; toast.classList.add('show'); setTimeout(() => toast.classList.remove('show'), 3000); }
}
...
if (cBtn) {
    cBtn.onclick = function(e) {
        ...
        if (!musicStarted && bgM) { bgM.play().then(() => { musicStarted = true; if (musicToggle) musicToggle.textContent = "🔊 Звук: Вкл"; }).catch(err => console.log(err)); }
        updateUI(); saveGame();
        if (gCont) {
